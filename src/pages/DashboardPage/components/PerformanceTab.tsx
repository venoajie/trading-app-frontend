...
import { Card, Skeleton, Box, Text, useMantineTheme } from '@mantine/core';
import {
  ResponsiveContainer,
  AreaChart,
  Area,
  ...
  if (active && payload && payload.length) {
    return (
      <Card withBorder shadow="sm" radius="md" p="sm">
        ...
        <Text
          fw={500}
        >{`Value: ${new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' }).format(payload[0].value ?? 0)}`}</Text>
      </Card>
    );
  }
  return null;
...
export function PerformanceTab() {
  ...
  const chartColor = theme.colors.blue[6];
